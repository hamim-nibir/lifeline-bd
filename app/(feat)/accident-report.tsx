...
type AccidentType = "Road Accident" | "Workplace Accident" | "Home Accident" | null;
type YesNo = "Yes" | "No";
...
export default function AccidentReportScreen() {
  ...
  const [accidentType, setAccidentType] = useState<AccidentType>(null);
  ...
  const handleSubmit = async () => {
    // Validation
    if (!accidentType) {
      Alert.alert("Required", "Please select an accident type.");
      return;
    }
    if (!numInjured) {
    ...
    try {
      ...
      const reportData = {
        ...
        accidentType,
        vehicleInfo: accidentType === "Road Accident"
          ? { vehicleType, numVehicles, plateNumber }
          : null,
        injuryDetails: { numInjured, severeInjury, bleeding },
        sceneSafety: { roadBlocked, fireRisk },
        location: {
      ...
      // Save report
      const reportRef = await addDoc(collection(db, "accidentReports"), reportData);
      ...
      await addDoc(collection(db, "notifications"), {
        ...
        body: `${nickname ?? "Someone"} reported a ${accidentType} at ${location.address}`,
        reportedBy: nickname ?? "Unknown",
        ...
        severity: severeInjury === "Yes" || bleeding === "Yes" ? "high" : "normal",
        read: false,
        createdAt: serverTimestamp(),
      });
      ...
      Alert.alert(
        "✅ Report Submitted",
        "Your accident report has been sent to all operators.",
  ...
  const RadioGroup = ({
    ...
    onChange: (v: any) => void; required?: boolean;
  }) => (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#f9fafb" }}>
      ...
      <View style={{
        ...
        <View>
          <Text style={{ color: "#fff", fontSize: 20, fontWeight: "700" }}>Accident Report</Text>
      ...
      >
        ...
        {/* Accident Type — required, starts unselected */}
        <View style={[cardStyle, !accidentType && { borderColor: "#fca5a5" }]}>
          <Text style={sectionTitleStyle}>
            Accident Type <Text style={{ color: "#dc2626" }}>*</Text>
          </Text>
          {!accidentType && (
          ...
          {(["Road Accident", "Workplace Accident", "Home Accident"] as const).map((type) => (
            <TouchableOpacity
              key={type}
          ...
          <View style={cardStyle}>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginBottom: 12 }}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  style={inputStyle} placeholder="e.g. Car, Bus"
                  placeholderTextColor="#9ca3af" value={vehicleType}
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  value={numVehicles} onChangeText={setNumVehicles}
                />
              </View>
            </View>
            <Text style={labelStyle}>Plate Number (Optional)</Text>
            <TextInput
              style={inputStyle} placeholder="e.g. DHA-1234"
        ...
        <View style={cardStyle}>
          ...
          <View style={{ flexDirection: "row", gap: 12, marginBottom: 4 }}>
            <View style={{ flex: 1 }}>
              <Text style={labelStyle}>
                Number of Injured <Text style={{ color: "#dc2626" }}>*</Text>
              </Text>
              <TextInput
                style={inputStyle} placeholder="e.g. 3"
                placeholderTextColor="#9ca3af" keyboardType="numeric"
                value={numInjured} onChangeText={setNumInjured}
              />
            </View>
            <View style={{ flex: 1 }}>
              <RadioGroup
                label="Severe injury?" options={["Yes", "No"]}
                value={severeInjury} onChange={setSevereInjury} required
              />
            </View>
          </View>
          <RadioGroup
            label="Bleeding / unconscious victims?" options={["Yes", "No"]}
            value={bleeding} onChange={setBleeding} required
          />
        </View>
        ...
        <View style={cardStyle}>
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            <View style={{ flex: 1 }}>
              <RadioGroup label="Road blocked?" options={["Yes", "No"]}
                value={roadBlocked} onChange={setRoadBlocked} />
            </View>
            <View style={{ flex: 1 }}>
              <RadioGroup label="Fire risk?" options={["Yes", "No"]}
                value={fireRisk} onChange={setFireRisk} />
            </View>
          </View>
        </View>
        ...
        <View style={cardStyle}>
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              borderWidth: location ? 1 : 0, borderColor: "#86efac",
          ...
          >
            {locLoading ? (
              ...
              <>
                <Text style={{ fontSize: 18 }}>📍</Text>
                <Text style={{ color: location ? "#15803d" : "#fff", fontWeight: "700", fontSize: 14 }}>
        ...
        <View style={{ flexDirection: "row", gap: 12, marginTop: 8 }}>
          ...
          <TouchableOpacity
            ...
            disabled={submitting}
            style={{
              flex: 2, backgroundColor: "#f97316", borderRadius: 14,
              paddingVertical: 16, alignItems: "center",
...
const sectionTitleStyle = { fontSize: 15, fontWeight: "700" as const, color: "#1f2937", marginBottom: 4 };
const labelStyle = { color: "#374151", fontSize: 13, fontWeight: "600" as const, marginBottom: 6, marginTop: 4 };
const inputStyle = {
